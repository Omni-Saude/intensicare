...
class StabilityAssessment(Base):
    ...
    score: Mapped[int] = mapped_column(
        Integer, nullable=False, comment="0-27 criteria in warning/critical"
    )
    severity: Mapped[str] = mapped_column(
        String(16), nullable=False, comment="estavel, atencao, critico"
    )
    criteria: Mapped[list[dict]] = mapped_column(
        JSONB, nullable=False, comment="27 StabilityCriterion dicts"
    )
