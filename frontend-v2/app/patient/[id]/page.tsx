        ...
        >
          ...
          <div className="flex items-center gap-3 mt-3">
            <button
              ...
              className="px-4 py-2 rounded-lg border text-sm font-medium hover:bg-[var(--semantic-surface-raised)] transition-colors"
            >
              Tentar novamente
            </button>
            <button
    ...
    <Layout>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-[var(--semantic-text-primary)]">{data.display_name}</h1>
            <p className="text-[var(--semantic-text-secondary)] text-sm mt-1">
              MPI: {data.mpi_id}
              ...
              <span className="inline-flex items-center gap-1 ml-3">
                {wsStatus === 'connected' ? (
                  ...
                  <WifiOff className="w-3 h-3 text-[var(--semantic-text-secondary)]" aria-hidden="true" />
                )}
                <span className="text-[10px] text-[var(--semantic-text-secondary)]">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 rounded-lg border bg-[var(--semantic-surface-raised)] hover:bg-[var(--semantic-surface-canvas)]"
            >
              <RefreshCw className={`w-4 h-4 text-[var(--semantic-text-secondary)] ${loading ? 'animate-spin' : ''}`} aria-hidden="true" />
            </button>
          </div>
        </div>
      </div>
      ...
      {latestVitals && (
        <div
          style={{ borderColor: 'var(--semantic-border-default)' }}
          className="bg-[var(--semantic-surface-raised)] rounded-xl border p-6 shadow-sm mb-6"
        >
          ...
          <div className="text-xs text-[var(--semantic-text-secondary)] mt-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div
          style={{ borderColor: 'var(--semantic-border-default)' }}
          className="bg-[var(--semantic-surface-raised)] rounded-xl border p-6 shadow-sm"
        >
          <h3 className="text-sm font-semibold text-[var(--semantic-text-secondary)] uppercase tracking-wider mb-4">
            <ClinicalTooltip term="MEWS">MEWS</ClinicalTooltip> Histórico
          </h3>
          {data.mews_history.length === 0 ? (
            <p className="text-sm text-[var(--semantic-text-secondary)]">Nenhum dado MEWS disponível</p>
          ) : (
        ...
        >
          ...
          {data.news2_history.length === 0 ? (
            <p className="text-sm text-[var(--semantic-text-secondary)]">Nenhum dado NEWS2 disponível</p>
          ) : (
            <div className="space-y-3 max-h-64 overflow-y-auto">
        ...
        >
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {data.vitals_history.map((point, idx) => (
              <div
                key={idx}
                className="flex items-center gap-4 py-2 border-b border-[var(--semantic-border-default)] last:border-0"
              >
                <span className="text-xs text-[var(--semantic-text-secondary)] w-20 flex-shrink-0">
...
function VitalItem({
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 text-xs text-[var(--semantic-text-secondary)] mb-1">
        {icon}
        {label}
      </div>
      <div
      ...
      >
        {value !== null && value !== undefined ? (
          <>
            ...
            {unit && <span className="text-sm font-normal text-[var(--semantic-text-secondary)] ml-0.5">{unit}</span>}
          </>
        ) : (
...
function ScoreTimelineRow({
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-[var(--semantic-border-default)] last:border-0">
      <span className="text-xs text-[var(--semantic-text-secondary)]">{formatDate(point.calculated_at)}</span>
      <div className="flex items-center gap-2">
        ...
        {point.trend === 'stable' && <span className="text-xs text-[var(--semantic-text-secondary)]">→</span>}
      </div>
    </div>
  );
}
