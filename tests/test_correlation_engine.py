...
class TestCorrelationSepsisAki:
    ...
    def test_tv8_breakthrough_severity_escalation(self, engine):
        ...
        assert result.fired is True, (
            f"TV-8: KDIGO stage 3 should fire correlation: {result.reason}"
        )
        assert result.severity == SeverityLevel.CRITICAL
...
class TestCorrelationQtcElectrolyte:
    ...
    def test_tv6_boundary_qtc_501_lytes_at_edge_no_fire(self, engine):
        ...
        assert result.fired is False, (
            f"TV-6: K=3.5 NOT <3.5, Mg=0.7 NOT <0.7 (strict) should not fire: "
            f"{result.reason}"
        )
...
class TestCorrelationExamRedund:
    ...
    def test_tv4_boundary_exactly_120h_no_fire(self, engine):
        ...
        assert result.fired is False, (
            f"TV-4: at exactly 120h (NOT < 120h strict) should not fire: "
            f"{result.reason}"
        )
    ...
    def test_tv5_boundary_119h_fires(self, engine):
        ...
        assert result.fired is True, (
            f"TV-5: 119h < 120h should fire: {result.reason}"
        )
...
class TestFullEvaluation:
    ...
    def test_convenience_function(self):
        ...
        result = evaluate_correlations("P-001", "E-001", {
            "sepsis_event": "sepsis.shock.detected",
            "sepsis_onset_at": 0,
            "aki_onset_at": 7200,
            "kdigo_stage": 2,
