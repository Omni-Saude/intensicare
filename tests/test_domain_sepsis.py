...
class TestHybridMode:
    ...
    def test_hybrid_screening_fires_triggers_lab(self, sepsis_service):
        ...
        screen_fired = any(
            r.alert_id == "ALERT-SEPSIS-SCREEN-01" and r.fired for r in nrt_result.alerts
        )
        ...
        organ_fired = any(
            r.alert_id == "ALERT-SEPSIS-ORGAN-02" and r.fired for r in lab_result.alerts
        )
    ...
    def test_hybrid_convenience_functions(self):
        ...
        mb_result = evaluate_sepsis_micro_batch(
            "P-003",
            {
                "qsofa": 2,
                "lactato_arterial": 2.4,
        ...
        organ = next(r for r in mb_result.alerts if r.alert_id == "ALERT-SEPSIS-ORGAN-02")
        assert organ.fired

        # Full convenience
        full_result = evaluate_sepsis(
            "P-004",
            {
                "qsofa": 0,
                "procalcitonina": 0.18,
        ...
        deesc = next(r for r in full_result.alerts if r.alert_id == "ALERT-SEPSIS-PCT-DEESC-06")
        assert deesc.fired
    ...
    def test_ppv_targets_all_above_fleet_floor(self, sepsis_service):
        ...
        for aid in SEPSIS_ALERT_IDS:
            ...
            if aid in allowed_below:
                assert target >= 0.58, f"{aid}: PPV target {target} below documented floor 0.58"
            else:
                assert target >= 0.60, f"{aid}: PPV target {target} below fleet floor 0.60"
...
class TestSepsisDomainService:
    ...
    def test_all_6_alerts_loaded(self, sepsis_service):
        ...
        for aid in SEPSIS_ALERT_IDS:
            assert aid in sepsis_service._definitions, f"Missing definition: {aid}"
    ...
    def test_screening_only(self, sepsis_service):
        ...
        result2 = sepsis_service.evaluate_screening_only(
            "P-002",
            {
                "qsofa": 2,
                "suspeita_infeccao_documentada": True,
    ...
    def test_organ_only(self, sepsis_service):
        ...
        result = sepsis_service.evaluate_organ_only(
            "P-001",
            {
                "qsofa": 2,
                "lactato_arterial": 2.4,
    ...
    def test_shock_only(self, sepsis_service):
        ...
        result = sepsis_service.evaluate_shock_only(
            "P-001",
            {
                "lactato_arterial": 4.2,
            },
    ...
    def test_mode_nrt_only_evaluates_screen_and_shock(self, sepsis_service):
        ...
        result = sepsis_service.evaluate(
            "P-001",
            {
                "qsofa": 2,
                "suspeita_infeccao_documentada": True,
