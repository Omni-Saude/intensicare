...
| PER-CARLOS-02 | "Menos de 3 falsos positivos por paciente-dia" | Per-patient-day **distribution** (both must hold): share of patient-days with > 3 dispositioned FP **≤ 5%**, and p95 FP/patient-day **< 3** | 'não procede' disposition events (US-22) counted per (patient, calendar-day) cell; patient-day cells from the occupancy census. Report the > 3-FP patient-day share and the p95 of the per-cell count (`ppv-ledger` `fp_patient_day_gt3_share` / `fp_patient_day_p95`); the fleet mean (total FP ÷ patient-days) is diagnostic only — blind to concentration on the sickest patients. Per unit and per alert type. | US-03, US-13..20, US-22, US-23, US-25, US-26 |
