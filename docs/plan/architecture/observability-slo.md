...
| 3 | **evaluate** | **5,000 ms** | **23,000 ms** (re-scoped: 30s − poll 1000 − normalize 500 − persist 500 − deliver_MVP 5000 = 23,000 ms; honours IMP-C-14 <5s MVP alert; not a standalone 30 s) | Yes | Run score algorithm + alert-definition predicates | `IMP-C-13`, `IMP-C-14` |
...
| | **Owned pipeline Σ (stages 1–5, Prod, linear upper bound)** | **9,000 ms = 9 s** | **= 30,000 ms** (MVP column sums exactly to the ceiling: 1000+500+23000+500+5000) | vs ceiling `VIS-C-09` **30 s** | Conservative — linear sum over-estimates true p95 since stages are not perfectly correlated | |
